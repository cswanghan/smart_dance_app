'use client';

import { useState, useEffect } from 'react';
import { Post } from '@/types';

export default function AdminPostsPage() {
  const [posts, setPosts] = useState<Post[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch('/api/v1/posts');
        if (!response.ok) {
          throw new Error('Failed to fetch posts');
        }
        const data = await response.json();
        setPosts(data.data);
      } catch (err: any) {
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };

    fetchPosts();
  }, []);

  if (loading) return <div className="text-center p-8">加载帖子...</div>;
  if (error) return <div className="text-center p-8 text-red-500">错误: {error}</div>;

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6">帖子管理</h1>
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="bg-gray-200 text-gray-700">
            <tr>
              <th className="py-3 px-4 text-left">ID</th>
              <th className="py-3 px-4 text-left">用户</th>
              <th className="py-3 px-4 text-left">内容</th>
              <th className="py-3 px-4 text-left">图片</th>
              <th className="py-3 px-4 text-left">点赞</th>
              <th className="py-3 px-4 text-left">评论</th>
              <th className="py-3 px-4 text-left">创建时间</th>
            </tr>
          </thead>
          <tbody className="text-gray-700">
            {posts.map(post => (
              <tr key={post.id} className="border-b border-gray-200 hover:bg-gray-50">
                <td className="py-3 px-4">{post.id}</td>
                <td className="py-3 px-4">{post.userName}</td>
                <td className="py-3 px-4">{post.content.substring(0, 50)}...</td>
                <td className="py-3 px-4">
                  {post.imageUrl ? (
                    <img src={post.imageUrl} alt="Post" className="w-16 h-16 object-cover rounded" />
                  ) : (
                    '无'
                  )}
                </td>
                <td className="py-3 px-4">{post.likes}</td>
                <td className="py-3 px-4">{post.comments}</td>
                <td className="py-3 px-4">{new Date(post.createdAt).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {posts.length === 0 && !loading && !error && (
        <div className="text-center mt-8 text-gray-500">暂无帖子数据。</div>
      )}
    </div>
  );
}
